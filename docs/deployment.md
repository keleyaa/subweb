...
Docker 需要 `APP_DOMAIN`、`API_DOMAIN`、`SHORT_DOMAIN` 这 3 个不同域名。3 个域名可以由同一台服务器的 Nginx、OpenResty、宝塔、1Panel、Cloudflare Tunnel 或其他入口接收，然后全部转发到 Gateway 的回环端口。MyUrls、Redis 和 SubConverter 不发布宿主机端口。
