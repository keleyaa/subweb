...
上述文件已于 2026-09-04 检查，确认可访问且使用 `[custom]` 格式。它们会引用其他公开规则集，因此使用前应阅读各自仓库及其下游来源的说明和许可证。

若要增加或移除预设，应同步更新 `public/conf/config.js`、`src/runtime/config.js`、本文件和对应测试。不要将用户订阅、访问令牌、私有地址或未经核验的个人配置作为默认预设提交。
