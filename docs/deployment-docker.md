...
分别校验 APP 与 SHORT hostname；只有 Gateway 将容器 8080 绑定到 loopback 的 `SUBWEB_PORT`。HTTPS、证书和
三域名路由由外部反向代理负责。
...
`configure.sh` 自动生成 Redis 密码和独立 IP 哈希秘密，无需手动填写。不要直接执行 `cat .env`；
只检查非秘密项时使用明确白名单：
...
`ghcr.io/keleyaa/subweb:sha-...` 是等价发布来源。脚本执行
...
外部代理应覆盖 `X-Forwarded-For`；只有明确配置该代理来源的
`TRUSTED_PROXY_CIDR` 后 Gateway 才信任它。不要把 Redis、MyUrls 或 SubConverter
端口暴露到公网。
...
不要在日志命令中附加 `.env` 内容。生产切换 v2 前先按
[`operations.md`](operations.md) 完成 Redis 盘点、备份和已批准 TTL 策略的迁移。
