...
生产 Compose 默认将 Redis 固定在稳定主线 `docker.io/library/redis:8-alpine`，而 SubConverter-Extended 与 MyUrls 继续跟随各自 `latest` 浮动标签；[`deploy/versions.lock.json`](../deploy/versions.lock.json) 保留已验证基线（验证日期 2026-08-18）：MyUrls 集成测试直接消费其 digest，其余服务作为回滚参考，并作为 `verify:locks` 校验的机器可读依据。需要冻结或受控回滚时在 `.env` 设置 `MYURLS_IMAGE`/`REDIS_IMAGE`/`SUBCONVERTER_IMAGE`/`SUBWEB_IMAGE`。
