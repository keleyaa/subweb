...
- [x] **步骤 1：先更新视觉源码契约并确认红灯**

  测试从“禁止任何 gradient”改为精确允许只在页面背景的静态 `linear-gradient`，并仍禁止工作区/控件内的装饰渐变。断言：

  - 品牌容器不再是 sticky 玻璃卡，文字水平居中，主题按钮在右上角/同一光学轴；
  - Home 宽度为 `52rem`，品牌、工作区和页脚光学中心一致；
...
- [x] **步骤 2：实现静态环境光和系统主题 token**

  `base.css` 定义浅色冷白/珍珠材料和深色烟蓝黑/亮蓝操作色；`body::before` 用覆盖整页的静态蓝、青、淡紫 linear gradient 做环境光，`pointer-events: none`，不加循环动画或离散光球。全局 canvas 仍有实色基底，使减少透明度时能直接取代 blur。

- [x] **步骤 3：精修品牌、主表面和来源页脚**
  ...
  - HomeView 删除重复品牌/营销式文案，以视觉隐藏的“订阅转换”二级标题保留语义；
...
- [x] 常用路径在一页完成，无模板、账号、数据卡、营销区或第二主面板。
- [x] 未转换时没有结果表面和短链操作；转换成功后只有一个转换结果和一个短链次级操作。
- [x] 剪贴板拒绝时 URL 仍可手动选择，页面不宣称成功复制。
- [x] 主题、reduced motion、reduced transparency、more contrast、键盘和 390px 视口全部通过真实浏览器测试。
- [x] 环境光是静态页面背景，工作区内不嵌套玻璃卡，无循环动画、弹跳或不可中断过渡。
- [x] 与 MyUrls 统一的是 token/材料/节奏/反馈，不复制 MyUrls DOM、图形资产或业务界面。
