...
Gateway 与所有外部生产镜像（Redis、SubConverter-Extended、Nginx 基础镜像、MyUrls）在 Compose 中默认跟随各自 `latest` 浮动标签；[`deploy/versions.lock.json`](../deploy/versions.lock.json) 保留集成测试与回滚使用的已验证基线（tag、commit、manifest digest 和平台 digest）。更新镜像时验证上游发布、许可证、架构摘要、容器健康、集成测试和漏洞扫描，并运行 `verify:integration:*` 确认无回归。

跟随 `latest` 意味着供应链不可完全复现，也无法依赖镜像 tag 精确回滚。对公开生产服务，需要受控回滚时应在 `.env` 中通过 `MYURLS_IMAGE`/`SUBWEB_IMAGE` 指定已验证 digest；其余镜像可通过发布流程保留的 SBOM、provenance 和源码 SHA 对应关系定位回滚点。发布流程仍应保留这些产物。
