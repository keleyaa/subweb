...
启用短链的四服务 profile 使用锁定的 MyUrls Rust v2.0.8、SubConverter 和 Redis production 镜像，并检查：
...
- MyUrls 只能使用 Gateway 的受限 `:25503` egress；`EGRESS_ALLOWED_HOSTS` 之外的主机在 DNS 解析前被拒绝（`403`），SubConverter 仍使用 `:25502` 订阅 egress；
