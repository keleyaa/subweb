...
- Node.js 24 与 npm 11
- OpenSSL 与 curl
- 三个未被占用的本地端口：Vite `5173`、Gateway `18081`、MyUrls `18082`
...
`npm run dev` 会先执行 Compose-first 依赖启动，再运行 Vite。依赖服务使用现行生产镜像和统一服务名：`gateway`、`subconverter`、`myurls`、`redis`。本地覆盖文件设置 `TURNSTILE_ENABLED=false`，避免本地开发依赖真实挑战服务；生产环境仍使用 `compose.yaml` 的 Cloudflare Turnstile 配置。
...
- MyUrls：`http://127.0.0.1:${LOCAL_MYURLS_PORT:-18082}/`
- Redis、SubConverter 不直接发布到宿主机
...
docker compose \
  ...
  -f compose.yaml -f compose.dev.yaml \
   logs --tail=100 gateway subconverter myurls redis
```
